...
cmake --build . --config Release --parallel %PROC% --target INSTALL
...
cd ecvl
REM git checkout tags/v0.3.3 REM Latest release
...
cmake --build . --config Debug --parallel %PROC% --target ALL_BUILD
cmake --build . --config Release --parallel %PROC% --target ALL_BUILD
echo Pipeline built
cd %UCP_PATH%