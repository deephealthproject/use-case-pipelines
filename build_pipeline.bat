...
mkdir deephealth & cd deephealth
...
cmake --build . --config %BUILD_TYPE% --target ALL_BUILD --parallel 4
cmake --build . --config %BUILD_TYPE% --target INSTALL
set ECVL_INSTALL_DIR=%UCP_PATH%/deephealth/ecvl/build/install
